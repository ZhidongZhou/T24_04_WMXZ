...
function idesign_body_class( $classes ) {
	...
	$hide_title = 0;
	$header_type = '';
	$header_layout_class = 'default-header';
	...
	// Header Style Class
	if ( get_theme_mod('header_style', '2') == '2' ) {
		$header_layout_class = 'max-header';
	} elseif ( get_theme_mod('header_style', '2') == '3' ) {
		$header_layout_class = 'navx-header';		
	}
	
	// Header Style Class page overwrite
	...
	$classes[] = $header_layout_class;
		
	if ( is_page_template( 'page_full-width.php' ) || is_page_template( 'page_bbuilder.php' ) || is_page_template( 'page_brizy.php' ) || is_page_template( 'page_elementor.php' ) ) {
...
function idesign_admin_notice_008() {
    ...
    if ( ! get_user_meta($user_id, 'idesign_ignore_notice_008') ) {
        ...
        echo '<div class="updated tx-dash-notice idesign-notice">'; 

		echo '<div class="nxn-image">';
		echo '<img src="'.get_template_directory_uri() . '/inc/txoc/small-images/portfolio.jpg" alt="nxn-screen">';		
		echo '</div>';
	
		echo '<div class="nxn-content">';
		printf(__('<h2 class="nxn-welcome">Welcome to I-DESIGN</h2>', 'i-design'));
		
		echo '<div class="nxn-desc">';
		printf(__('<p>A multipurpose theme with WooCommerce and RTL Support. I-DESIGN is always compatible with the latest versions of WordPress. It comes with multiple <a href="%scustomize.php?autofocus[section]=headeroptions" target="_blank">Header Types</a>. </p>', 'i-design'), admin_url() );
		printf(__('<p class="nxn-start">Start your website with a beautifully designed I-DESIGN demo.</p>', 'i-design'));				
		echo '</div>';
		
		printf(__('<a href="%1$s" target="_blank" class="txoc-import txadslarge txocwiz" style="margin-top: 16px; margin-right: 12px;">I-DESIGN Setup Wizard</a>', 'i-design' ), $about_url);	
		printf(__('<a href="%1$s" target="_blank" class="tx-dismiss after-setup" style="margin-top: 16px;">Adjustments After Demo Import</a>', 'i-design' ), $support_url);
		printf(__('<a href="%1$s" class="tx-dismiss after-setup" style="margin-top: 16px;">Dismiss This Notice</a><div class="clear"></div>', 'i-design' ), '?idesign_notice_ignore_008=0');					
		
		printf(__('<a href="%1$s" class="tx-dashnotice-close">Dismiss</a><div class="clear"></div>', 'i-design' ), '?idesign_notice_ignore_008=0');				
        echo '</div>';
