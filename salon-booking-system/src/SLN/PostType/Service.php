...
//        );
    ...
    public function manage_column($column, $post_id)
    {
		...
		echo '<div class="sln-service-unit" data-value="'.$obj->getUnitPerHour().'"></div>';
            ...
            case 'sln_days_off' :
                ...
                break;
            case 'taxonomy-sln_service_category' :
                $this->categoryColumn($post_id);
                break;
	...
	?>
		...
		<div class="col-sm-12 sln-inline-edit-service-fields">
			...
			<div class="col-sm-3">
				...
				<?php SLN_Form::fieldNumeric('sln_service_unit', null, array('max' => 100)); ?>
			    </label>
			</div>
		    </div>
		    <div class="row">
    ...
    private function categoryColumn($post_id){
        $taxonomy = 'sln_service_category';
        ...
        if ( is_array( $terms ) ) {
            ...
            foreach ( $terms as $t ) {
                ...
                $posts_in_term_qv['post_type'] = $this->getPostType();
                ...
                $label = __($label,'salon-booking-system');
                
                $url = add_query_arg( $posts_in_term_qv, 'edit.php' );
