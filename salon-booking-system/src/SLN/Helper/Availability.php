...
class SLN_Helper_Availability
{
    ...
    public function getTimes(Date $date)
    {
        ...
        $from = $hb->getFromDate();
        $to = $hb->getToDate();
	foreach (SLN_Func::getDateMinutesIntervals($date->getDateTime(), $this->getItemsWithoutServiceOffset()) as $time) {
	...
	if (empty($times)) {
	    $times   = array();
	    $times[] = new SLN_DateTime($startAt->format('H:i'));
	}
    ...
    public function returnValidatedServices(array $servicesIds)
    {
        ...
        $isServicesCountPrimaryServices = $this->settings->get('is_services_count_primary_services');
		...
		if ($isServicesCountPrimaryServices) {
			...
			if (count($_validated) >= $servicesCount) {
			    break;
			}
		    }
		} else {
    ...
    public function checkEachOfNewServicesForExistOrder($order, $newServices, $altMode = false)
    {
        ...
        $servicesCount			= $s->get('services_count');
        $isServicesCountPrimaryServices = $s->get('is_services_count_primary_services');
		...
		if($servicesCount) {
			...
			if(count($services) >= $servicesCount) {
			    $ret[$service->getId()] = array(sprintf(__('You can select up to %d items', 'salon-booking-system'), $servicesCount));
			    continue;
			}
		    }
    ...
    {
	return SLN_Func::getDateMinutesIntervals($date->getDateTime(), $this->getItemsWithoutServiceOffset());
    }
