...
(function($) {
	...
	function getExtentedOption(cal, option_name) {
		...
		else if(option_name == 'first_day' && fromOptions != null ){ return fromOptions;}
		else {
			if(fromOptions != null) {
				return fromOptions;
			}
	...
	function eventOverlap(start,duration,events){
		var overlap = 0;
		var end = start+duration;
		events.forEach(function(event){
			var eventEnd = event[0]+ event[1];
			if(!(  end <= event[0]  || start >= eventEnd )) overlap = event[2]+1;
	...
	Calendar.prototype._render = function() {
		...
		if(first_day) data.months = data.months.concat(data.months.splice(0,first_day))
	...
	Calendar.prototype._calculate_hour_minutes_daily = function(data) {
		...
		if ($self.options._assistants_mode) {
			...
			$.each(data.events, function(k, e) {
				...
				$.each(e.items, function(j, item) {
					service_start = service_end;
                                        ...
                                        var durationMs = duration[0] === '00' && duration[1] === '00' ?
					 salon_default_duration  *(60 * 1000) :
					 (parseInt(duration[0])*60*60 + parseInt(duration[1])*60)*1000;
                                        ...
                                        var breakDurationMs = (parseInt(break_duration[0]) * 60 * 60 + parseInt(break_duration[1]) * 60) * 1000;

                                        var addServiceToAssistants = function (service_start, service_end, classes) {
                                            ...
                                            if(!(att_id in assistants)){
                                                    assistants[att_id] = [];
                                                    assistants[att_id].eventCount = 0;
                                            }else{
                                                    var checkifcontained =  eventOverlap(obj.top, obj.lines, assistants[att_id])
                                                    if(checkifcontained){
                                                            assistants[att_id].eventCount = Math.max(assistants[att_id].eventCount,checkifcontained);
                                                            eventVal[2]= checkifcontained ;
                                                    }
                                            }
                                            assistants[att_id].push(eventVal)
                                        }

                                        if (breakDurationMs > 0) {

                                            service_end = service_start + Math.floor(durationMs / 2);
                                            addServiceToAssistants(service_start, service_end, ' break-down');

                                            service_start = service_end + breakDurationMs ;
                                            service_end   = service_start + Math.ceil(durationMs / 2);
                                            addServiceToAssistants(service_start, service_end, ' break-up');

                                        } else {
                                            service_end = service_start + durationMs;
                                            addServiceToAssistants(service_start, service_end);
                                        }
			...
			var attIds = Object.keys(assistants);
			var i = 0;
			var ret = [];
			attIds.forEach(function(att_id){
				...
				i = i+1+assistants[att_id].eventCount;
				ret.push(att_id);
				if(assistants[att_id].eventCount) ret = ret.concat(Array.apply(null, Array(assistants[att_id].eventCount)).map(function() { return 'undefined'; }));
			})
			...
			ret = ret.slice(current_page * on_page, (current_page+1) * on_page);
			...
			$.each(data.events, function(k, e) {
				...
				var service_end   = service_start;
                                ...
                                $.each(e.items, function(j, item) {
                                    ...
                                    var addServiceToEvent = function (service_start, service_end, classes) {
                                            var objs = $self.applyAdvancedFilters([{
                                                id: 	  	 e.id,
                                                title:           k === 0 ? e.title : '',
                                                from:            k === 0 ? e.from : '',
                                        ...
                                        if (breakDurationMs > 0) {
                                            ...
                                            service_end = service_start + Math.floor(durationMs / 2);
                                            addServiceToEvent(service_start, service_end, ' break-down no-border-top');

                                            service_start = service_end + breakDurationMs ;
                                            service_end   = service_start + Math.ceil(durationMs / 2);
                                            addServiceToEvent(service_start, service_end, ' break-up no-border-top');

                                        } else {
                                            service_end = service_start + durationMs;
                                            addServiceToEvent(service_start, service_end, ' no-border-top');
                                        }
                                });
                                ...
                                $.each(event, function (j, obj) {
                                    ...
                                    while(!added) {
                                            ...
                                            if (grid[row_rounded][col] == undefined) {
                                                    ...
                                                    var offset_max = parseFloat((obj.lines).toFixed(4));
...
*/
	...
	Calendar.prototype._update_day_prepare_sln_booking_editor = function() {
		...
		setTimeout(function() {
		    ...
		    $('[data-cal-date="'+ date +'"]').closest('.cal-month-day').trigger('mouseenter').trigger('click');
